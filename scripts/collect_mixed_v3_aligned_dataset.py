#!/usr/bin/env python3
"""Collect and merge a mixed aligned v3 training dataset.

This is the mixed-dataset wrapper for
``nash_skills.v2.collect_data_v3_aligned``. It keeps the same random-vs-random
and fixed-vs-random allocation used by the v2 wrapper, but records decision
states with the next skill pair that is applied after each net crossing.
"""
...
from nash_skills.v2 import collect_data_v3_aligned

DEFAULT_OUTPUT = "data/rallies_5skill_v3_aligned_mixed_10k.pkl"
...
def collect_segment(
    *,
    ...
    with suppress_return_lines(enabled=not verbose_returns):
        collect_data_v3_aligned.collect(
...
def run(args) -> list[dict[str, Any]]:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="mixed_v3_aligned_collect_"))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Collect a mixed random/fixed-random aligned v3 dataset.")
