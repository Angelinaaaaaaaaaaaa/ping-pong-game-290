...
import scripts.collect_mixed_v3_aligned_dataset as mixed
...
def _install_fake_collect(monkeypatch, extra_discard=False):
    ...
    monkeypatch.setattr(mixed.collect_data_v3_aligned, "collect", fake_collect)
    return calls


def test_default_output_is_aligned_v3_mixed_dataset():
    args = mixed.build_parser().parse_args([])

    assert args.random_rallies == 7000
    assert args.fixed_random_rallies == 3000
    assert args.output == "data/rallies_5skill_v3_aligned_mixed_10k.pkl"
...
def test_script_does_not_reimplement_rollout_logic():
    ...
    assert "collect_data_v3_aligned.collect(" in source
