    ...
    with open(LOG_P_PATH, "w", newline="") as f:
        ...
        for epoch in range(n_epochs):
            ...
            with torch.no_grad():
                ...
                target_p1_right = model1(X11) - model1(X01)  # right vs left (opp=right)
                target_p1_left  = model1(X10) - model1(X00)  # right vs left (opp=left)
                # Player 2 unilateral deviation constraints
                target_p2_right = model2(X11) - model2(X10)  # right vs left (ego=right)
                target_p2_left  = model2(X01) - model2(X00)  # right vs left (ego=left)

            # Constraint 1: player 1 deviation with opp=right
            optimizer_p.zero_grad()
            loss_p = criterion(
                model_p(X11) - model_p(X01),
                target_p1_right,
            )
            loss_p.backward()
            optimizer_p.step()
