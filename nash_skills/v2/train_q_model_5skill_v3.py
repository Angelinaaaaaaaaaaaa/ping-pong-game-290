"""
Train Q-value + potential models for the 5-skill pipeline, but train the
potential model using same-state per-sample counterfactual constraints.

Compared with train_q_model_5skill_v2.py:
  - Q-model training is unchanged.
  - Potential training samples a base batch of states, clones that same batch
    into counterfactual skill assignments, and matches per-sample Q deltas
    instead of minibatch means.
  - model_p is kept in eval() mode so BatchNorm uses frozen running stats.
